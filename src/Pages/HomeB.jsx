...
import { addToCard } from '../slice/counterSlice';
  ...
  const HomeB = () => {
    const  {id} =useParams()
    const   pro1=b.find((item)=>item.id===parseInt(id))
    ...
    return (
      <div >
       <div className='flex shadow-xl/30 flex bg-white w-320 ml-75 gap-8 mt-8 mb-20'>
                    ...
                    <div className='flex'>
                         <button onClick={() => dispatch(addToCard(pro1))}  className="flex  cursor-pointer gap-2 bg-yellow-400 ml-3 border hover:bg-yellow-300 text-black font-semibold py-2 px-6 rounded mt-4 "><LuShoppingBag className="w-35 h-5 " />ADD TO BAG</button>
                  ...
                  <p className='ml-6 mt-2 font-semibold text-2xl '>Save extra with these offers</p>
                          ...
                          <div>
                             <button onClick={()=> navigate("/paymentpage",{ state: { price: pro1.Price } })} className=" ml-7 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 w-58 px-4 rounded mt-7 cursor-pointer">Buy Now </button>

                              {/* <button
                               onClick={() => navigate("/","/paymentpage", { state: { price } })}
                              className="bg-red-500 text-white px-9 py-2 rounded ml-[-24rem] hover:bg-red-600 cursor-pointer"
                                >
                            Buy
                             </button> */}
                             <button onClick={() => dispatch(addToCard(pro1))}  className="ml-2 bg-#008C2D-600 bg-red-500 hover:bg-green-700 text-white font-semibold w-54 py-2 px-4 rounded mt-7 cursor-pointer"> Add to Cart
                            </button>
                          </div>
