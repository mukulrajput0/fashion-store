...
import { addToCard } from '../slice/counterSlice';
  ...
  const HomeBag = () => {
    const  {id} =useParams()
    const pro1=bag1.find((item)=>item.id===parseInt(id))
    ...
    return (
      <div className='flex shadow-xl/30 flex bg-white w-320 ml-75 gap-8 mt-8 mb-20'>
             ...
             <div className='flex'>
                  <button onClick={()=> dispatch(addToCard(pro1))} className="flex  gap-2 bg-yellow-400 ml-3 border hover:bg-yellow-300 text-black font-semibold py-2 px-6 rounded mt-4 cursor-pointer "><LuShoppingBag className="w-35 h-5 " />ADD TO BAG</button>
